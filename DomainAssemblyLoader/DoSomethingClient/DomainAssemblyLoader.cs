...
namespace DoSomethingClient
{
    public class DomainAssemblyLoader : MarshalByRefObject
    {
        ...
        public Result LoadFile(string path, Input data)
        {
            ...
            var typesWithAttribute = types.Where(t => t.GetCustomAttributes(typeof(DoSomethingAttribute), true).Length > 0);
            Type typeDoesntImplement = typesWithAttribute.FirstOrDefault(t => !(t is IDoSomething));

            Type type = null;
            
            type = typeDoesntImplement;
            ...
            result = mi.Invoke(instance, new object[] { data }) as Result;
